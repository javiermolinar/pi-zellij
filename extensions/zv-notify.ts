...
type NotifyBackend = "unknown" | "osascript" | "notify-send" | "unavailable";
...
function getNotifyLevelFromEnv(): NotifyLevel {
	const value = process.env.PI_ZV_NOTIFY_LEVEL?.trim().toLowerCase();
...
function getDefaultBackend(): NotifyBackend {
...
export default function zvNotifyExtension(pi: ExtensionAPI) {
	const thresholdMs = getNumberFromEnv("PI_ZV_NOTIFY_THRESHOLD_MS", DEFAULT_THRESHOLD_MS);
	const debounceMs = getNumberFromEnv("PI_ZV_NOTIFY_DEBOUNCE_MS", DEFAULT_DEBOUNCE_MS);
	const notifyLevel = getNotifyLevelFromEnv();
	const title = process.env.PI_ZV_NOTIFY_TITLE || "Pi";

	let runState = createEmptyRunState();
	let lastNotificationAt = 0;
	let lastNotificationKey = "";
	let notifyBackend: NotifyBackend = "unknown";

	const sendNotification = async (subtitle: string, body: string): Promise<{ ok: boolean; error?: string }> => {
		if (notifyBackend === "unknown") {
			notifyBackend = getDefaultBackend();
		}
		if (notifyBackend === "unavailable") {
			return { ok: false, error: "desktop notifications are unavailable on this platform" };
		}
		...
		const result =
			notifyBackend === "osascript"
				? await pi.exec(
						...
						[
							"-e",
							`display notification ${escapeAppleScriptString(body)} with title ${escapeAppleScriptString(title)} subtitle ${escapeAppleScriptString(subtitle)}`,
						],
						{ timeout: NOTIFY_TIMEOUT_MS },
				  )
				: await pi.exec("notify-send", [`${title} — ${subtitle}`, body], { timeout: NOTIFY_TIMEOUT_MS });

		if (result.killed) {
			return { ok: false, error: "notification command timed out" };
		}
		if (result.code !== 0) {
			const error = result.stderr.trim() || result.stdout.trim() || `notification command exited with code ${result.code}`;
			if (error.includes("not found") || error.includes("ENOENT")) {
				notifyBackend = "unavailable";
			}
